...
while (members.includes('유노윤호')&& members.includes('최강창민')&&members.includes('영웅재중')&& members.includes('믹키유천')&& members.includes('시아준수')) {

let adjustmember = prompt(`수정할 이름을 입력하세요`)

let newmember = prompt(`새로운 이름을 입력하세요!`)

let ad_idx = members.indexOf(adjustmember)

if(ad_idx === -1){
    alert(`${adjustmember}은 잘못된 이름입니다.`)
}
else{
    members.splice(ad_idx,1,newmember)
    alert(`수정완료! \n 남은멤버 : [ ${members}]`)
}
}